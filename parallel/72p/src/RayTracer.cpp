...
Color RayTracer::traceRays(int x,int y) {
       // columnsCompleted++;
      ...
      color  = castRayForPixel(x, y, raysCast);
         //image.pixel(x, y, castRayForPixel(x, y, raysCast));
      //}
...
Color RayTracer::castRayForPixel(int x, int y, uint64_t& raysCast) const {
   ...
   for (int x = 0; x < superSamples; x++) {
      for (int y = 0; y < superSamples; y++) {
         ...
         color = color + (castRayAtPoint(imagePlanePoint, raysCast) * sampleWeight);
      }
   }
...
Color RayTracer::castRayAtPoint(const Vector& point, uint64_t& raysCast) const {
   Color color;
   ...
   for (int i = 0; i < depthComplexity; i++) {
      ...
      color = color + (castRay(viewRay, raysCast) * (1 / (float)depthComplexity));
   }

   return color;
}

Color RayTracer::castRay(const Ray& ray, uint64_t& raysCast) const {
   raysCast++;
   Intersection intersection = getClosestIntersection(ray);

   if (intersection.didIntersect) {
      return performLighting(intersection, raysCast);
   } else {
      return Color();
   }
}

bool RayTracer::isInShadow(const Ray& ray, double lightDistance) const {
   Intersection intersection = getClosestIntersection(ray);

   return intersection.didIntersect && intersection.distance < lightDistance;
}

Intersection RayTracer::getClosestIntersection(const Ray& ray) const {
...
Color RayTracer::performLighting(const Intersection& intersection,
 uint64_t& raysCast) const {
   ...
   Color reflectedColor = getReflectiveRefractiveLighting(intersection, raysCast);

   return ambientColor + diffuseAndSpecularColor + reflectedColor;
}

Color RayTracer::getAmbientLighting(const Intersection& intersection, const Color& color) const {
   return color * 0.2;
}
...
Color RayTracer::getDiffuseAndSpecularLighting(const Intersection& intersection,
 const Color& color) const {
   ...
   for (vector<Light*>::const_iterator itr = lights.begin(); itr < lights.end(); itr++) {
      Light* light = *itr;
...
Color RayTracer::getReflectiveRefractiveLighting(const Intersection& intersection,
  uint64_t& raysCast) const {
   ...
   if (reflectivePercentage > 0) {
      ...
      reflectiveColor = castRay(reflectedRay, raysCast) * reflectivePercentage;
   }
   ...
   if (refractivePercentage > 0) {
      ...
      refractiveColor = castRay(refractedRay, raysCast) * refractivePercentage;
   }
...
Vector RayTracer::reflectVector(Vector vector, Vector normal) const {
